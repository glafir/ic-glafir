...
autocomplete :airport, :city_rus, :limit => 25, :extra_data => [:name_rus, :city_eng, :iata_code], :display_value => :apdata
...
#   end
  ...
  def show
    ...
    @weathers = @airport.weathers.where("created_at >= ?", "#{Time.now - 168.hours}").order(:created_at)
  ...
  def update
    authorize @airport
    flash[:notice] = "The airport  #{@airport.id} was updated!" if @airport.update(airport_params) && !request.xhr?
    respond_with(@airport)
  end
